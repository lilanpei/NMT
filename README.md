...
2) We use only Mircoblog set in UM-Corpus to reduce the memory and time consumption of training as the serve cannot fit the model for whole dataset, and for the part of full scale dataset, it already needs more than one day to train an epoch on the serve, but it will be worthwhile testing on the full scale dataset.  
